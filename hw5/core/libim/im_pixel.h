...
class IM_TYPE
{
 public:
   ...
   // Image sampling **
   PIXEL Sample(int x, int y, char bound = 'w');
   PIXEL Sample(float x, float y, char bound = 'w');
   ...
   void ThresholdAutomatic(float minval, float maxval, bool invert = false);
   void RemoveIsolated(int window);
   ...
   void UnsharpMasking(float sigma, float weight);
   void UnsharpMasking(int iterations, float weight, char bound = 'z');
   void Power(float gamma);
   void Cubic(float param);
   void Stretch(int r1, int s1, int r2, int s2);
   void Wallis(int radius, float A = 0.0, float B = 0.0);

   // Smoothing operations
   void Average(int xdim, int ydim);
   void Binomial(char bound = 'z');
   void Binomial(int iterations, char bound = 'z');
   ...
   void Translate(float dx, float dy, char bound = 'z');
   void Rotate(float angle, int cx, int cy, char bound = 'z');
   void Scale(float sx, float sy, int cx, int cy, char bound = 'z');
   void Shear(float sx, float sy, int cx, int cy, char bound = 'z');
   ...
   void Corner();
   void Watershed(float sigma, im_short & region);
   ...
   float Product(IM_TYPE & in2);
   float Product(IM_TYPE & in2, int dx, int dy);
   float Product(IM_TYPE & in2, int search);
   float Difference(IM_TYPE & in2);
   float Difference(IM_TYPE & in2, int dx, int dy);
   float Difference(IM_TYPE & in2, int search);
   float GraphDistance(IM_TYPE &graph2, IM_TYPE &dist1, IM_TYPE &dist2);
   float GraphDistance(IM_TYPE &graph2, IM_TYPE &dist1, IM_TYPE &dist2, int dx, int dy);
   float GraphDistance(IM_TYPE &graph2, IM_TYPE &dist1, IM_TYPE &dist2, int search);
   float GraphMatch(IM_TYPE &in2);
   float GraphMatch(IM_TYPE &in2, int dx, int dy);
   float GraphMatch(IM_TYPE &in2, int search);
   void ShowDifference(IM_TYPE &in1, IM_TYPE &in2, int shift);
   ...
   void DrawLine(float M, float B, int size, PIXEL value);
   ...
   // Advanced operations -------------------
   void BlobColor();
   void BlobColor(int min_size, int max_size);
   void BlobColor(int x, int y, int color, int & blob_size);
