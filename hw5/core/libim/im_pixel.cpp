...
//============================================================
PIXEL IM_TYPE::Sample(int x, int y, char bound)
{
   // Wrap around pixel coordinates
   if (bound == 'w')
   {
      if (y < 0)     y = y % Ydim + Ydim;
      if (y >= Ydim) y = y % Ydim;
      if (x < 0)     x = x % Xdim + Xdim;
      if (x >= Xdim) x = x % Xdim;
   }
...
//============================================================
PIXEL IM_TYPE::Sample(float x, float y, char bound)
{
   ...
   PIXEL Value00 = Sample((int)x+0, (int)y+0, bound);
   PIXEL Value01 = Sample((int)x+0, (int)y+1, bound);
   PIXEL Value10 = Sample((int)x+1, (int)y+0, bound);
   PIXEL Value11 = Sample((int)x+1, (int)y+1, bound);
...
//============================================================
bool IM_TYPE::WriteJpg(string filename)
{
   char charname[STRLEN];
   strcpy(charname, filename.c_str());
   return WriteJpg(charname);
}
...
//============================================================
bool IM_TYPE::ReadJpg(string filename)
{
   char charname[STRLEN];
   strcpy(charname, filename.c_str());
   return ReadJpg(charname);
}
...
void IM_TYPE::ThresholdAutomatic(bool invert)
{
   ...
   for (int i = 1; i < Range - 1; i++)
   {
      ...
      float mean2 = (Total[Range - 1] - Total[i]) 
                  / (Histo[Range - 1] - Histo[i]);
...
//============================================================
void IM_TYPE::ThresholdAutomatic(float minval, float maxval, bool invert)
{
   // Calculate pixel range and histogram
   PIXEL Min, Max;
   MinMax(Min, Max);
...
//============================================================
void IM_TYPE::RemoveIsolated(int window)
{
   // Remove isolated white points
   for (int y=window; y<Ydim-window; y++)
   for (int x=window; x<Xdim-window; x++)
   if (Data2D[y][x] == 1)
   {
      // Count number of white points
      int white = 0;
      for (int dy=y-window; dy<y+window; dy++)
      for (int dx=x-window; dx<x+window; dx++)
         if (Data2D[dy][dx] == 1) white++;

      // Remove isolated white point
      if (white < window)
         Data2D[y][x] = 0;
   }
}
...
void IM_TYPE::Trim(float fraction)
{
   ...
   // Find trim window
   int low_index = 0;
   while ((low_index < Range) && (Histo[low_index] < fraction*NumPixels))
      low_index++;
   int high_index = Range-1;
   while ((high_index >= 0) && (Histo[high_index] > (1-fraction)*NumPixels))
      high_index--;

   // Window image
   PIXEL low = (PIXEL) (Min + low_index);
   PIXEL high = (PIXEL) (Min + high_index);
   Window(low, high);
...
//============================================================
void IM_TYPE::UnsharpMasking(float sigma, float weight)
{
   // Parameter checking 
   if (sigma < 1)
      sigma = 1;
   ...
   for (int index = 0; index < NumPixels; index++)
   {
      PIXEL Mask = Data1D[index] - blur.Data1D[index];
      Data1D[index] += weight * Mask;
      if (Data1D[index] > Max) Data1D[index] = Max;
      if (Data1D[index] < Min) Data1D[index] = Min;
   }
}

//============================================================
void IM_TYPE::UnsharpMasking(int iterations, float weight, char bound)
{
   // Parameter checking 
   if (iterations < 1)
      iterations = 1;
   ...
   IM_TYPE blur(*this);
   blur.Binomial(iterations, bound);
...
//============================================================
void IM_TYPE::Wallis(int radius, float A, float B)
{
   // Parameter checking
   if (radius < 1)
      radius = 1;
   ...
   for (int y = 0; y < Ydim; y++)
	 ...
	 // Perform enhancement
	 Data2D[y][x] = Stddev * (Data2D[y][x] - mean) / (stddev + A) 
                      + Mean * B + mean * (1 - B);
         if (Data2D[y][x] < Min) Data2D[y][x] = Min;
         if (Data2D[y][x] > Max) Data2D[y][x] = Max;
      }
}
...
//============================================================
void IM_TYPE::Binomial(char bound)
{
   // Copy input image
   IM_TYPE in(*this);
   ...
   for (int y = 0; y < Ydim; y++)
      ...
      {
	 Data2D[y][x] = (in.Sample(x-1, y-1, bound) +
			 in.Sample(x,   y-1, bound) * 2 +
			 in.Sample(x+1, y-1, bound) +

			 in.Sample(x-1, y, bound) * 2 +
			 in.Sample(x,   y, bound) * 4 +
			 in.Sample(x+1, y, bound) * 2 +

			 in.Sample(x-1, y+1, bound) +
			 in.Sample(x,   y+1, bound) * 2 +
			 in.Sample(x+1, y+1, bound) + 8) / 16;
      }
...
//============================================================
void IM_TYPE::Binomial(int iterations, char bound)
{
   // Perform Binomial smoothing
...
//============================================================
void IM_TYPE::Translate(float dx, float dy, char bound)
{
   // Copy input image
   IM_TYPE in(*this);
   ...
   for (int y = 0; y < Ydim; y++)
	 ...
	 float xpos = x - dx;
	 float ypos = y - dy;
	 Data2D[y][x] = in.Sample(xpos, ypos, bound);
      }
}

//============================================================
void IM_TYPE::Rotate(float angle, int cx, int cy, char bound)
{
   // Copy input image
   IM_TYPE in(*this);
...
//============================================================
void IM_TYPE::Scale(float sx, float sy, int cx, int cy, char bound)
{
   // Copy input image
   IM_TYPE in(*this);
...
//============================================================
void IM_TYPE::Shear(float sx, float sy, int cx, int cy, char bound)
{
   // Copy input image
   IM_TYPE in(*this);
...
void IM_TYPE::FitRectangle(int &x1, int &x2, int &y1, int &y2, int search, char method)
{
   ...
   if (x1 < 0) x1 = 0;
   if (y1 < 0) y1 = 0;
   if (x2 > Xdim-1) x2 = Xdim-1;
   if (y2 > Ydim-1) y2 = Ydim-1;
...
//============================================================
void IM_TYPE::Watershed(float sigma, im_short & region)
{
   // Create gradient image
   im_float gradient(*this);
   gradient.Gaussian(sigma);
   gradient.Gradient();
   gradient.Greymap(0, 100);
   gradient.Window(1, 100);
...
void IM_TYPE::NoiseImpulse(char type, float fraction)
{
   ...
   PIXEL Min, Max;
   MinMax(Min, Max);
   PIXEL Salt = (PIXEL) (Min + 0.9 * (Max - Min));
   PIXEL Pepper = (PIXEL) (Min + 0.1 * (Max - Min));
...
//============================================================
float IM_TYPE::Product(IM_TYPE & in2)
{
   float product = 0;
   int skip = 10;
   ...
   if ((Xdim == in2.Xdim) && (Ydim == in2.Ydim))
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	    product += Data2D[y][x] * in2.Data2D[y][x];
   }
   ...
   else
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    ...
	    product += Data2D[y][x] * in2.Data2D[yy][xx];
	 }
   }
   return (product / (Xdim * Ydim));
}

//============================================================
float IM_TYPE::Product(IM_TYPE & in2, int dx, int dy)
{
   float product = 0;
   int skip = 10;
   ...
   if ((Xdim == in2.Xdim) && (Ydim == in2.Ydim))
   {
      for (int y = skip; y < Ydim - skip; y++)
	 for (int x = skip; x < Xdim - skip; x++)
            product += Data2D[y+dy][x+dx] * in2.Data2D[y][x];
   }
   ...
   else
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    int yy = (int) (0.5 + y * yscale);
	    int xx = (int) (0.5 + x * xscale);
	    product += Data2D[y+dy][x+dx] * in2.Data2D[yy][xx];
	 }
   }
   return (product / (Xdim * Ydim));
}
...
//============================================================
float IM_TYPE::Product(IM_TYPE & in2, int search)
{
   // Search for minimum product
   float product = Product(in2);
   for (int dx = -search; dx <= search; dx++)
   for (int dy = -search; dy <= search; dy++)
   if ((dx != 0) || (dy != 0))
   {
      float temp = Product(in2, dx, dy);
      if (product > temp)
         product = temp;
   }
   return product;
...
//============================================================
float IM_TYPE::Difference(IM_TYPE & in2)
{
   float difference = 0;
   int skip = 10;
   ...
   return (difference / (Xdim * Ydim));
}

//============================================================
float IM_TYPE::Difference(IM_TYPE & in2, int dx, int dy)
{
   float difference = 0;
   int skip = 10;
   ...
   if ((Xdim == in2.Xdim) && (Ydim == in2.Ydim))
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    float temp = Data2D[y+dy][x+dx] - in2.Data2D[y][x];
	    if (temp < 0) temp = -temp;
	    difference += temp;
	 }
   }
   ...
   else
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    int yy = (int) (0.5 + y * yscale);
	    int xx = (int) (0.5 + x * xscale);
	    float temp = Data2D[y+dy][x+dx] - in2.Data2D[yy][xx];
	    if (temp < 0) temp = -temp;
	    difference += temp;
	 }
   }
...
//============================================================
float IM_TYPE::Difference(IM_TYPE & in2, int search)
{
   // Search for minimum difference
   float difference = Difference(in2);
   for (int dx = -search; dx <= search; dx++)
   for (int dy = -search; dy <= search; dy++)
   if ((dx != 0) || (dy != 0))
   {
      float temp = Difference(in2, dx, dy);
      if (difference > temp)
         difference = temp;
   }
   return difference;
...
//============================================================ 
float IM_TYPE::GraphDistance(IM_TYPE &graph2, IM_TYPE &dist1, IM_TYPE &dist2)
{
   float result = 0;
   int skip = 10;

   // Check graph and dist images have same dimensions
   if (Xdim != dist1.Xdim || Ydim != dist1.Ydim)
      printf("Error graph1 image not same size as dist1 image\n");
   if (graph2.Xdim != dist2.Xdim || graph2.Ydim != dist2.Ydim)
      printf("Error graph2 image not same size as dist2 image\n");

   // Handle input images with same dimensions
   if ((Xdim == graph2.Xdim) && (Ydim == graph2.Ydim))
   {
      for (int y = skip; y < Ydim - skip; y++)
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    result += Data2D[y][x] * dist2.Data2D[y][x]
	           + graph2.Data2D[y][x] * dist1.Data2D[y][x];
	 }
   }

   // Handle input images with different dimensions
   else
   {
      float xscale = (float) (graph2.Xdim - 1) / (float) (Xdim - 1);
      float yscale = (float) (graph2.Ydim - 1) / (float) (Ydim - 1);
      for (int y = skip; y < Ydim - skip; y++)
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    int yy = (int) (0.5 + y * yscale);
	    int xx = (int) (0.5 + x * xscale);
	    result += Data2D[y][x] * dist2.Data2D[yy][xx]
	           + graph2.Data2D[yy][xx] * dist1.Data2D[y][x];
	 }
   }
   return (result / (Xdim * Ydim));
}

//============================================================
float IM_TYPE::GraphDistance(IM_TYPE &graph2, IM_TYPE &dist1, IM_TYPE &dist2, int dx, int dy)
{
   float result = 0;
   int skip = 10;
   ...
   if ((Xdim == graph2.Xdim) && (Ydim == graph2.Ydim))
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    result += Data2D[y+dy][x+dx] * dist2.Data2D[y][x]
	           + graph2.Data2D[y][x] * dist1.Data2D[y+dy][x+dx];
	 }
   }
   ...
   else
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    int yy = (int) (0.5 + y * yscale);
	    int xx = (int) (0.5 + x * xscale);
	    result += Data2D[y+dy][x+dx] * dist2.Data2D[yy][xx]
	           + graph2.Data2D[yy][xx] * dist1.Data2D[y+dy][x+dx];
	 }
   }
   return (result / (Xdim * Ydim));
}
...
//============================================================
float IM_TYPE::GraphDistance(IM_TYPE &graph2, IM_TYPE &dist1, IM_TYPE &dist2, int search)
{
   // Search for best result
   float result = GraphDistance(graph2, dist1, dist2);
   for (int dx = -search; dx <= search; dx++)
   for (int dy = -search; dy <= search; dy++)
   if ((dx != 0) || (dy != 0))
   {
      float temp = GraphDistance(graph2, dist1, dist2, dx, dy);
      if (result > temp)
         result = temp;
   }
   return result;
...
//============================================================
float IM_TYPE::GraphMatch(IM_TYPE & in2)
{
   int match = 1;
   int total = 1;
   int skip = 10;
   ...
   if ((Xdim == in2.Xdim) && (Ydim == in2.Ydim))
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
         {
	    if (Data2D[y][x]>0 && in2.Data2D[y][x]>0) match++;
	    if (Data2D[y][x]>0 || in2.Data2D[y][x]>0) total++;
         }
   }
   ...
   else
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    int yy = (int) (0.5 + y * yscale);
	    int xx = (int) (0.5 + x * xscale);
	    if (Data2D[y][x]>0 && in2.Data2D[yy][xx]>0) match++;
	    if (Data2D[y][x]>0 || in2.Data2D[yy][xx]>0) total++;
	 }
   }
...
//============================================================
float IM_TYPE::GraphMatch(IM_TYPE & in2, int dx, int dy)
{
   int match = 1;
   int total = 1;
   int skip = 10;
   ...
   if ((Xdim == in2.Xdim) && (Ydim == in2.Ydim))
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
         {
	    if (Data2D[y][x]>0 && in2.Data2D[y+dy][x+dx]>0) match++;
	    if (Data2D[y][x]>0 || in2.Data2D[y+dy][x+dx]>0) total++;
         }
   }
   ...
   else
   {
	 ...
	 for (int x = skip; x < Xdim - skip; x++)
	 {
	    int yy = (int) (0.5 + y * yscale);
	    int xx = (int) (0.5 + x * xscale);
	    if (Data2D[y][x]>0 && in2.Data2D[yy+dy][xx+dx]>0) match++;
	    if (Data2D[y][x]>0 || in2.Data2D[yy+dy][xx+dx]>0) total++;
	 }
   }
   return (float)match / (float)total;
}
...
//============================================================
float IM_TYPE::GraphMatch(IM_TYPE &im2, int search)
{
   // Search for best result
   float result = GraphMatch(im2);
   for (int dx = -search; dx <= search; dx++)
   for (int dy = -search; dy <= search; dy++)
   if ((dx != 0) || (dy != 0))
   {
      float temp = GraphMatch(im2, dx, dy);
      if (result < temp)
         result = temp;
   }
   return result;
...
// JOHN
//============================================================
void IM_TYPE::ShowDifference(IM_TYPE &in1, IM_TYPE &in2, int shift)
{
   // Create result image
   int xdim = (in1.Xdim + in2.Xdim)/2;
   int ydim = (in1.Ydim + in2.Ydim)/2;
   IM_TYPE copy1, copy2;
   copy1.Copy(in1);
   copy2.Copy(in2);
   copy1.Interpolate(xdim, ydim);
   copy2.Interpolate(xdim, ydim);
   copy2.Translate(shift, shift);
...
//============================================================
void IM_TYPE::DrawLine(float M, float B, int size, PIXEL value)
{
   int x1 = 0;
   int y1 = M * x1 + B;
   int x2 = Xdim-1;
   int y2 = M * x2 + B;
   DrawLine(x1,y1, x2,y2, size, value);
}
...
//============================================================
void IM_TYPE::BlobColor(int min_size, int max_size)
{
   ...
   // Calculate size of blobs
   for (int y = 0; y < Ydim; y++)
      for (int x = 0; x < Xdim; x++)
	 Size[(int)(Data2D[y][x])]++;

   // Assign new blob numbers
   ...
   for (int y = 0; y < Ydim; y++)
      for (int x = 0; x < Xdim; x++)
         Data2D[y][x] = Blob[(int)(Data2D[y][x])];

   // JOHN
   return;

   // Recursive search for blobs
   int color = 2;
   const int MAX_COLOR = 32767;
   for (int y = 0; y < Ydim; y++)
      for (int x = 0; x < Xdim; x++)
...
//============================================================
void IM_TYPE::BlobColor(int x, int y, int color, int & blob_size)
{
   ...
   if (Data2D[y][x] == 1)
   {
      ...
      if ((x - 1 >= 0) && (Data2D[y][x-1] == 1))
	 BlobColor(x - 1, y, color, blob_size);
      if ((y - 1 >= 0) && (Data2D[y-1][x] == 1))
	 BlobColor(x, y - 1, color, blob_size);
      if ((x + 1 < Xdim) && (Data2D[y][x+1] == 1))
	 BlobColor(x + 1, y, color, blob_size);
      if ((y + 1 < Ydim) && (Data2D[y+1][x] == 1))
	 BlobColor(x, y + 1, color, blob_size);

      if ((x - 1 >= 0) && (y - 1 >= 0) && (Data2D[y-1][x-1] == 1))
	 BlobColor(x - 1, y - 1, color, blob_size);
      if ((x + 1 < Xdim) && (y - 1 >= 0) && (Data2D[y-1][x+1] == 1))
	 BlobColor(x + 1, y - 1, color, blob_size);
      if ((x + 1 < Xdim) && (y + 1 < Ydim) && (Data2D[y+1][x+1] == 1))
	 BlobColor(x + 1, y + 1, color, blob_size);
      if ((x - 1 >= 0) && (y + 1 < Ydim) && (Data2D[y+1][x-1] == 1))
	 BlobColor(x - 1, y + 1, color, blob_size);
   }
}
