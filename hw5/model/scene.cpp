 ...
 *
 *  Description:
 *  Creates and owns the ray-tracing scene objects, lighting, and simple animation updates.
 *
 *  Dependencies:
 *  scene.h
 *
...
Scene::Scene()
{
    ...
    // gray sphere in the center of the scene
    ...
    graySphere->setMaterial(grayColor, 0.3f, 0.5f, 0.15f, 10.0f);
    addObject(std::move(graySphere));

    // yellow sphere near the gray sphere
    // it will rotate around the gray sphere
    ...
    positionCubeTwo.set(-8.0f, 1.0f, 20.0f);
    cubeTwo->setGeometry(positionCubeTwo, 2.0f);    
    ColorRGB cubeTwoColor;
    cubeTwoColor.set(255, 0, 0);
    cubeTwo->setMaterial(cubeTwoColor, 0.3f, 0.5f, 0.15f, 10.0f);
