...
public:
    ...
    virtual bool intersect(Ray3D ray, Point3D &p, Vector3D &n) const;
    ...
    float getKs() const;
    float getKp() const;
    void setMaterial(ColorRGB c, float ka, float kd, float ks, float kp);
    virtual ColorRGB getSurfaceColor(Point3D hitPoint) const;
private:
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
